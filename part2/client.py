...
class ChatClient:
    ...
    def __init__(self, window: Tk):
        ...
        Label(self.window, text=f"Client @ port#{self.client_port}").grid(row=0, column=0)
    ...
    def send_message(self, event=None):
        ...
        if new_message.strip():
            ...
            self.update_chat_history(f"{self.client_id}: {new_message}", align="center")
    ...
    def receive_message(self):
        while True:
            try:
                ...
                if server_message:
                    self.window.after(0, self.update_chat_history, f"{server_message}", "left")
